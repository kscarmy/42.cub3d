...
/*   Updated: 2022/08/07 09:05:36 by guderram         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	printf("\nft_hc_add : x %f y %f x %f y %f dx %f dy %f\n", w->h->hx, w->h->hy, w->h->hcx, w->h->hcy, w->dirx, w->diry);
	w->h->hx += (w->h->hcx * w->dirx);
	w->h->hy += (w->h->hcy * w->diry);
	w->h->hr += w->h->hcr;
	printf("ft_hc_add : x %f y %f x %f y %f dx %f dy %f\n", w->h->hx, w->h->hy, w->h->hcx, w->h->hcy, w->dirx, w->diry);
}
